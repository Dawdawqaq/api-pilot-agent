 ...
 * Zero-dependency, modern ES2024 utilities
 */
...
export const Utils = {
  ...
  /**
   * Copy text to clipboard with micro-interaction feedback
   */
  async copyToClipboard(text, successMsg = '已复制到剪贴板') {
    try {
  ...
  showToast(message, type = 'info', duration = 3200) {
    ...
    toast.innerHTML = `
      <span style="flex-shrink:0; margin-top:2px;">${this.getIcon(iconName, 16)}</span>
      <div style="flex:1; line-height:1.4;">${this.escapeHtml(message)}</div>
      <button style="background:none; border:none; color:var(--text-muted); cursor:pointer; padding:0 2px;" aria-label="关闭">
        ${this.getIcon('x', 14)}
      </button>
    `;
  ...
  /**
   * Micro SVG Icons (Feather / Lucide Style)
   */
  getIcon(name, size = 16) {
    const icons = {
      ...
      'terminal': `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="4 17 10 11 4 5"></polyline><line x1="12" y1="19" x2="20" y2="19"></line></svg>`,
      ...
      'copy': `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>`,
